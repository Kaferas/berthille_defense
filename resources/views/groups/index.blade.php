...
@section('content')
    <div class="container mt-3 pt-4">
        <div class="row">
            <h3 class="col-md-3"> Listes des Groupes</h3>
            <form action="{{ route('groups.index') }}" method="get" class="col-md-7">
                <div class="row">
            ...
            <a href="{{ route('groups.create') }}" class="btn btn-light m-2 bg-light col-md-1 text text-light">➕</a>
        </div>
        <div class="row mt-3">
            <div class="bg-secondary rounded h-100 p-4">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        @endif
                        <tbody>
                            @foreach ($groups as $ky => $item)
                                <tr>
                                    <th scope="row">{{ $ky + 1 }}</th>
                                    <td>{{ $item->name_group }}</td>
                                    <td>{{ $item->deleted_status == 0 ? 'Active' : 'Supprime' }}</td>
                                    <td>
                                        <div class="m-2">
                                            <button onclick="viewGroup('{{ $item->id }}')"
                                                class="btn btn-square btn-warning m-2 viewEleve" title="Voir Niveau"><i
                                                    class="fa fa-eye"></i></button>
                                            <a href="{{ route('groups.edit', $item) }}" type="button"
                                                class="btn btn-square btn-info m-2" title="Editer Niveau"><i
                                                    class="fa fa-edit"></i></a>
                                            <button type="button" onclick="deleteGroup('{{ $item->id }}')"
    ...
    <script>
        ...
        function viewGroup(id) {
            ...
            $.ajax({
                ...
                success: (data) => {
                    $(".NiveauNom").html(data.name_group)
                }
            })
        }
    </script>
