...
@section('content')
    <div class="container mt-3 pt-4">
        <div class="row">
            <h3 class="col-md-3"> Listes des Utilisateurs</h3>
            <form action="{{ route('users.index') }}" method="get" class="col-md-7">
                <div class="row">
            ...
            <a href="{{ route('users.create') }}" class="btn btn-light m-2 bg-light col-md-1 text text-light">➕</a>
        </div>
        <div class="row mt-3">
            <div class="bg-secondary rounded h-100 p-4">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        @endif
                        <tbody>
                            @foreach ($users as $ky => $item)
                                <tr>
                                    <th scope="row">{{ $ky + 1 }}</th>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ $item->email }}</td>
                                    <td>Admin/Ecrivain</td>
                                    <td>
                                        <div class="m-2">
